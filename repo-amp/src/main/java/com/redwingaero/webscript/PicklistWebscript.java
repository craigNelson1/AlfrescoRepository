...
public class PicklistWebscript extends DeclarativeWebScript{
    ...
    protected void process(WebScriptRequest req, Map<String, Object> model) throws Exception
    {
		...
		query.append("=" + VendorDatalist.CONTENT_MODEL_PREFIX + ":" + ContentModel.PROP_TITLE.getLocalName() + ":\"" + picklistName + "\"");
		query.append(" AND TYPE:\"" + VendorDatalist.DATALIST_MODEL_PREFIX + ":" + VendorDatalist.TYPE_DATALIST.getLocalName() + "\"");
		...
		else
		{
			...
			query2.append(" AND TYPE:\"" + VendorDatalist.REDWINGAERO_VALUE_ASSISTANCE_MODEL_PREFIX + ":" + VendorDatalist.TYPE_VALUE_ASSISTANCE_LIST_ITEM.getLocalName() + "\"");
			...
			searchParameters2.addSort("@" + VendorDatalist.PROP_SORT_ORDER, true);
	        ...
	        if (StringUtils.isNotBlank( loadLabels ))
	        {
				...
				else
				{
					for (NodeRef nodeRef : rs2.getNodeRefs())
					{
						...
						valueLabelPairs.put((String)props.get(VendorDatalist.PROP_VALUE),(String)props.get(ContentModel.PROP_NAME));
					}
	        ...
	        else
	        {
				...
				for (NodeRef nodeRef : rs2.getNodeRefs())
				{
					...
					{
						picklistItems.add( new PicklistItem((String)props.get(ContentModel.PROP_NAME), (String)props.get(VendorDatalist.PROP_VALUE)));
					}
				}
